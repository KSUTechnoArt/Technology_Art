...
const double backMotorL_Kp = 10.2; // 比例制御のための定数
const double backMotorR_Kp = -10.2; // 比例制御のための定数
...
void duringDriveCar() {
  if(distance_ToF <= 50 || distance_PHOTO > 2500) {
    ...
    if(value_determine_RL > 0) {
      ...
      writeMotorResister(backMotorL, 45, 0x01);
